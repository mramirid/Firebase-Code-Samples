...
class MainActivity : AppCompatActivity() {
    ...
    override fun onStart() {
        super.onStart()
        notebookReference.addSnapshotListener(this) { queryDocumentSnapshots, exception ->
            if (exception != null) {
            ...
            for (documentChange in queryDocumentSnapshots?.documentChanges!!) {
                ...
                val oldIndex = documentChange.oldIndex // Ketika baru menambah oldIndex bernilai -1
                val newIndex = documentChange.newIndex // Ketika menghapus newIndex bernilai -1

                when(documentChange.type) {
                    ADDED -> tv_data.append(
                        getString(R.string.indexChange, "Added", id, oldIndex, newIndex)
                    )
                    MODIFIED -> tv_data.append(
                        getString(R.string.indexChange, "Modified", id, oldIndex, newIndex)
                    )
                    REMOVED -> tv_data.append(
                        getString(R.string.indexChange, "Removed", id, oldIndex, newIndex)
                    )
                }
            }
        }
